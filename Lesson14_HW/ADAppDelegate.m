...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    //print the first and the second half of the string
    NSLog(@"\nFirst half:\n%@", [text substringToIndex:stringLength/2]);
    NSLog(@"\nSecond half:\n%@", [text substringFromIndex:stringLength/2]);
    */
    
    //array with the separator ". "
    NSArray *sentencesArray = [text componentsSeparatedByString:@". "];
    //NSLog(@"\nSentences array:\n%@", sentencesArray);

    /********** Student Level **********/
    
    //mutable array to store found strings
    NSMutableArray *searchResultArray = [[NSMutableArray alloc] init];
    
    for(NSString* string in sentencesArray){
        ...
        //if ([[string rangeOfString:@"NSString"] location] != NSNotFound) {
            ...
            [searchResultArray addObject:string];
        }
    }
    
    //NSLog(@"\nArray with strings containing \"NSString\":\n%@", searchResultArray);
    
    
    /********** Master Level **********/
    
    //the array to collect reversed sentences
    NSMutableArray *reversedSentence = [[NSMutableArray alloc] init];

    for(NSString* string in searchResultArray){
        
        NSArray *words = [string componentsSeparatedByString:@" "];
        NSMutableArray *wordsReversed = [[NSMutableArray alloc] init];
        
        for(int i = ([words count]-1); i>=0; i--){

            [wordsReversed addObject:[words objectAtIndex:i]];
        }
        
        [reversedSentence addObject:[wordsReversed componentsJoinedByString:@" "]];
    }
         
    //NSLog(@"\nReversed sentences: \n%@", reversedSentence);
    
    
    /********** Superman Level REVISED **********/
    
    NSArray *wordsFromText = [text componentsSeparatedByString:@" "];
    NSMutableArray *changedWords = [[NSMutableArray alloc] init];
    
    for(NSString *word in wordsFromText){
        
        NSUInteger firstIndex = NSNotFound;
        NSUInteger lastIndex = NSNotFound;
        
        //convert to mutable string
        NSMutableString *string = [NSMutableString stringWithFormat:@"%@", word];
        
        //set the index of the 1st character to change
        if([string length] > 0){
            
            for(NSUInteger i = 0; i < [string length]; i++){
                
                if([[NSCharacterSet letterCharacterSet] characterIsMember:[string characterAtIndex:i]]){
                    firstIndex = i;
                    break;
                }
            }
        }
        //set the index of the last char to change
        if([string length] > 1){
            
            for(NSUInteger i = [string length]-1; i > 0; i--){
                
                if([[NSCharacterSet letterCharacterSet] characterIsMember:[string characterAtIndex:i]] && i !=firstIndex){
                    lastIndex = i;
                    break;
                }
            }
        ...
        if (firstIndex != NSNotFound) {
                string = [self replaceCharAtIndex:firstIndex inString:string withCase:ADCaseValueLower];
        }
        if (lastIndex != NSNotFound){
                string = [self replaceCharAtIndex:lastIndex inString:string withCase:ADCaseValueUpper];
        }
        [changedWords addObject:string];
    }
    
    text = [changedWords componentsJoinedByString:@" "];
    NSLog(@"%@", text);
    return YES;
}

-(NSMutableString*) replaceCharAtIndex:(NSUInteger) index inString:(NSMutableString*) string withCase:(ADCaseValue) caseValue{

    NSString *letter;
    if (caseValue == ADCaseValueLower) {
        letter = [[string substringWithRange:NSMakeRange(index, 1)] lowercaseString];
    }else if (caseValue == ADCaseValueUpper){
        letter = [[string substringWithRange:NSMakeRange(index, 1)] uppercaseString];
    }
    
    [string replaceCharactersInRange:NSMakeRange(index, 1) withString:letter];
    return string;
}
